...
type DiscoveryQuery = Pick<IngestionQuery, "sessionsDir" | "claudeProjectsDir" | "providers">

type ConflictResolution = {
  accepted: LoadedSource[]
  conflicts: IngestionConflict[]
}

type DiscoverSourcesPort = (query: DiscoveryQuery) => Promise<SessionSource[]>
type ReadSourceContentPort = (source: SessionSource) => Promise<string>
type HashContentPort = (content: string) => string
type ConflictResolverPort = (loadedSources: LoadedSource[]) => ConflictResolution
type ParseSessionPort = (source: SessionSource, content: string) => Session | null
type ClockPort = () => Date

export type IngestionDeps = {
  discoverSources: DiscoverSourcesPort
  readSourceContent: ReadSourceContentPort
  hashContent: HashContentPort
  resolveConflicts: ConflictResolverPort
  parseSession: ParseSessionPort
  now: ClockPort
}

export type IngestionService = {
  ingest(query?: IngestionQuery): Promise<IngestionResult>
}
...
function resolveWindow(query: IngestionQuery, now: Date): TimeWindow {
...
async function loadSources(
  ...
  deps: Pick<IngestionDeps, "readSourceContent" | "hashContent">,
  ...
  for (const source of sources) {
    let content = ""
    try {
      content = await deps.readSourceContent(source)
    } catch {
      continue
    }

    loaded.push({ source, content, hash: deps.hashContent(content) })
  }

  return loaded
}

function defaultResolveConflicts(loadedSources: LoadedSource[]): ConflictResolution {
...
function defaultParseSession(source: SessionSource, content: string): Session | null {
  if (source.provider === "claude") return parseClaudeSessionContent(source, content)
  return parsePiSessionContent(source, content)
}

const defaultIngestionDeps: IngestionDeps = {
  discoverSources: (query) =>
    discoverSessionSources({
      piSessionsDir: query.sessionsDir,
      claudeProjectsDir: query.claudeProjectsDir,
      providers: query.providers,
    }),
  readSourceContent: (source) => readFile(source.path, "utf-8"),
  hashContent: defaultHashContent,
  resolveConflicts: defaultResolveConflicts,
  parseSession: defaultParseSession,
  now: () => new Date(),
}

async function ingestWithDeps(deps: IngestionDeps, query: IngestionQuery): Promise<IngestionResult> {
  const window = resolveWindow(query, deps.now())

  const discovered = await deps.discoverSources({
    sessionsDir: query.sessionsDir,
    claudeProjectsDir: query.claudeProjectsDir,
    providers: query.providers,
  })

  const loadedSources = await loadSources(discovered, deps)
  const { accepted, conflicts } = deps.resolveConflicts(loadedSources)

  const sessions: Session[] = []

  for (const source of accepted) {
    const session = deps.parseSession(source.source, source.content)
    if (!session) continue
...
export function createIngestionService(partialDeps: Partial<IngestionDeps> = {}): IngestionService {
  ...
  return {
    ingest(query: IngestionQuery = {}) {
      return ingestWithDeps(deps, query)
    },
  }
}
...
export async function ingestLocalSessions(query: IngestionQuery = {}): Promise<IngestionResult> {
  return defaultIngestionService.ingest(query)
}
